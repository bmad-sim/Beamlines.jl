...
@testset "Beamlines.jl" begin
    ...
    @test m2.species_ref == branch2.beamlines[2].line[1].species_ref
...
@testset "FieldSourceParams" begin
  field_source = (x, y, s, t) -> (x, y, s, t)
  additional_field = (x, y, s, t) -> (0, 0, 0, 0)
  whole = Quadrupole(FieldSourceParams=FieldSourceParams(; field_source))
  individual = Quadrupole(; field_source)
  @test whole.FieldSourceParams isa FieldSourceParams
  @test whole.field_source === field_source
  @test whole ≈ individual
  @test isnothing(whole.additional_field)
  @test isnothing(Quadrupole().FieldSourceParams)
  @test isnothing(Quadrupole().field_source)
  @test isnothing(Quadrupole().additional_field)
  @test :FieldSourceParams in propertynames(whole)
  @test :field_source in propertynames(whole)
  @test :additional_field in propertynames(whole)

  individual.FieldSourceParams = FieldSourceParams(; additional_field)
  @test individual.additional_field === additional_field
  @test isnothing(individual.field_source)
  @test !(individual ≈ whole)
  individual.additional_field = field_source
  @test individual.additional_field === field_source
  individual.FieldSourceParams = nothing
  @test isnothing(individual.FieldSourceParams)

  copied = deepcopy_no_beamline(whole)
  @test copied ≈ whole
  @test copied.FieldSourceParams !== whole.FieldSourceParams
  context = Context(strength=2.0)
  deferred = FieldSourceParams(field_source=DefExpr{Float64}(c -> c.strength))
  @test Beamlines.deval(deferred, context).field_source == 2.0
  @test deferred.field_source isa DefExpr
  dual = FieldSourceParams(field_source=ForwardDiff.Dual(2.0, 1.0))
  @test scalarize(dual).field_source === 2.0
end
